...
//拼接dsn percona1/ppercona1234@10.16.16.26:1521/svdp
...
func getInfo(ctx context.Context, db *sql.DB) (*plugin.Info, error) {
	info := &plugin.Info{}
	err := db.QueryRowContext(ctx, "select host_name,1521 as port,version from v$instance").Scan(&info.Hostname, &info.Port, &info.Version)
	if err != nil {
		return nil, err
	}
	info.Distro = "ORACLE"
